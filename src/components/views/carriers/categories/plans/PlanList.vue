  ...
  <section class="content">
    ...
    <data-table :item="item" 
      ...
      <template slot="list" slot-scope="{row}">
        ...
        <td>{{row.plan_regno}}</td>
        <td>{{row.carrier_name}}</td>
        <td>{{row.category_name}}</td>
        <td>{{row.data_type}}</td>
        <td>{{row.plan_name}}</td>
        <td>{{row.base_talktime}}</td>
        <td>{{row.base_message}}</td>
        <td>{{row.base_data}}</td>
        <td>{{row.reg_date}}</td>
...
export default {
  ...
  methods: {
    ...
    getPlans(params = null) {
      var token = localStorage.getItem("ACCESS_TOKEN")
      this.$http.defaults.headers.common['x-access-token'] = token
      this.$http.get(this.$host + this.$rootpath + "/plans?" + params)
      .then(result => {
        this.dataMap = result.data.data.plan_list
      }).catch(e => {
